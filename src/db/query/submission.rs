...
pub async fn delete_submission(db: &Db, id: i32) -> QueryResult<()> {
    db.run(move |conn| {
...
pub async fn delete_user_submissions(db: &Db, id: i32) -> QueryResult<()> {
    db.run(move |conn| {
        diesel::delete(submissions::table)
            .filter(submissions::user.eq(id))
            .execute(conn)
...
pub async fn delete_challenge_submissions(db: &Db, id: i32) -> QueryResult<()> {
    db.run(move |conn| {
        diesel::delete(submissions::table)
            .filter(submissions::challenge.eq(id))
            .execute(conn)
...
pub async fn delete_user_challenge_submissions(
    db: &Db,
    user: i32,
    challenge: i32,
) -> QueryResult<()> {
